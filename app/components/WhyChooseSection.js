...
const AMENITIES = [
  ...
  { index: "05", title: "Future Appreciation", description: "Properties selected for strong growth potential." },
  { index: "06", title: "Customer First", description: "Personalized support from enquiry to ownership." },
  { index: "07", title: "Premium Living", description: "Modern spaces designed for comfort and lifestyle." },
  { index: "08", title: "Lasting Trust", description: "Building relationships beyond the property purchase." },
];
...
export default function WhyChooseSection() {
  ...
  useEffect(() => {
    ...
    const getAllAnimatedEls = () => [
      headingLineRef.current,
      ...wordRefs.current.filter(Boolean),
      imageRef.current,
      ...sectionEl.querySelectorAll("[data-fade],[data-reveal]"),
    ].filter(Boolean);

    const forceVisible = () => {
      const els = getAllAnimatedEls();
      els.forEach((el) => {
        gsap.set(el, { clearProps: "all", opacity: 1, y: 0, yPercent: 0, scale: 1 });
      });
    };

    const safetyTimeout = setTimeout(forceVisible, 900);
    ...
    const ctx = gsap.context(() => {
      ...
      mm.add(
        ...
        (context) => {
          ...
          const fades = sectionEl.querySelectorAll("[data-fade]");
          if (fades.length) {
            gsap.fromTo(
              fades,
              { opacity: 0, y: 18 },
          ...
          const gridEl = sectionEl.querySelector("[data-amenities-grid]");
          const cards = sectionEl.querySelectorAll("[data-reveal]");
  ...
  return (
    ...
    >
      {/* Rendered as a plain, always-injected <style> tag instead of
          styled-jsx — styled-jsx requires Next's build-time transform to
          turn <style jsx> into real scoped CSS, and in this project's
          pipeline that transform wasn't running, so the block was being
          shipped as inert markup and none of these rules ever reached
          the page (same root cause hit on the GPD build). This approach
          renders a real <style> element regardless of toolchain. */}
      <style dangerouslySetInnerHTML={{ __html: WHY_CHOOSE_CSS }} />
